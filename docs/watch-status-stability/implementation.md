...
Et delvist svar med gyldig status/graf og manglende AGP anvendes straks; alene AGP får sin egen stigende retry-frist, som ikke udskyder status/grafens cachefrist eller en manuel opdatering.
...
**Første Xcode-kontrol og afgrænsede opfølgninger**

Den første Codemagic-verifikation `6aa55412d6ec5518ed891cde` på `23463917` kørte på Xcode 26.6: testloggen indeholder 402 unikke testudfald, 400 beståede og to fejlede. Resultatet blokerede udgivelse. Testen for nye data under en aktiv opbygning afdækkede, at en gammel cachekvittering kunne fjerne en nyere ventende ændring. Dubletfjernelse og push kræver nu, at den opbyggede cacheversion svarer til den aktuelle dataversion. Den oprindelige test er bevaret og suppleret med en regressionstest.

Den eksisterende legacy-journaltest overskred sin kvitteringsfrist, mens indholdet kunne genindlæses korrekt. Lagringens completion ventede på hovedkøen efter UI-notifikation. En separat lille rettelse giver completion på en egen serialiseret kø efter faktisk lagring; kun den ikke-autoritative legacy receipt-ledger-opdatering går tilbage til main. En deterministisk test holder main optaget og kræver både kvittering og genindlæselig lagring. Ingen BLE- eller runtime-politik ændres.

CI eksporterer nu Xcodes summary og testtræ også ved testfejl og bevarer den fejlede exitstatus. Efter den ene autoriserede upload læser `check-apple-processing.py` den præcise app/version/build fra det kontrollerede arkiv via den eksisterende Apple-integration. Polling stopper efter 15 minutter og starter aldrig en ny upload. Intern Apple-status, gruppetilknytning og installation holdes adskilt; læsescriptet kan ikke bekræfte gruppetilknytning eller fysisk installation. Den kompakte `APPLE_BUILD_OBSERVATION` i post-publish-loggen er beviset, også hvis artefakter allerede er indsamlet.
