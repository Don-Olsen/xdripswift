...
class TreatmentEntryAccessor {
    ...
    /// Given an NSManagedObjectID, returns the matching TreatmentEntry if it still exists.
    /// - parameters:
    ///     - objectID : the managed object identifier
    func getTreatment(objectID: NSManagedObjectID) -> TreatmentEntry? {
        var treatmentEntry: TreatmentEntry?

        coreDataManager.mainManagedObjectContext.performAndWait {
            treatmentEntry = try? coreDataManager.mainManagedObjectContext.existingObject(with: objectID) as? TreatmentEntry
        }
