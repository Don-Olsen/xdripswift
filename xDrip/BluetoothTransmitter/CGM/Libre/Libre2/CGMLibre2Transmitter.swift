...
class CGMLibre2Transmitter: BluetoothTransmitter, CGMTransmitter {
    ...
    private let CBUUID_Service_Libre2 = Libre2DirectConstants.serviceUUIDString
    
    /// receive characteristic
    private let CBUUID_ReceiveCharacteristic_Libre2 = Libre2DirectConstants.receiveCharacteristicUUIDString
    
    /// write characteristic
    private let CBUUID_WriteCharacteristic_Libre2 = Libre2DirectConstants.writeCharacteristicUUIDString
    
    /// how many bytes should we receive from Libre 2
    private let expectedBufferSize = Libre2DirectConstants.encryptedFrameLength
    ...
    /// The latest validated NFC-prepared test session generated by this transmitter.
    private var preparedWatchTestSessionID: UUID?
    ...
    override func startScanning() -> BluetoothTransmitter.startScanningResult {
        guard watchTestOwnershipSessionID == nil else {
            return .other(reason: "Prepared Libre test sensor is owned by Apple Watch")
        }
    ...
    override func centralManager(_ central: CBCentralManager, didConnect peripheral: CBPeripheral) {
        ...
        guard watchTestOwnershipSessionID == nil else { return }
    ...
    @nonobjc func canReleasePreparedSensorToWatch(_ session: LibreWatchTestSession) -> Bool {
        guard session.isValid,
              (preparedWatchTestSessionID == nil || preparedWatchTestSessionID == session.id),
              watchTestOwnershipSessionID == nil,
    ...
    /// Retains identity but stops this collector's scan/reconnect loop before Watch is acknowledged.
    @nonobjc func releasePreparedSensorToWatch(
        _ session: LibreWatchTestSession,
        completion: @escaping (Bool) -> Void
    ) {
        guard canReleasePreparedSensorToWatch(session) else {
            completion(false)
            return
        }
        watchTestOwnershipSessionID = session.id
        pauseConnectionWithoutReconnect { completion(true) }
    }

    @nonobjc func returnPreparedSensorToPhone(sessionID: UUID) {
        guard watchTestOwnershipSessionID == sessionID else { return }
        watchTestOwnershipSessionID = nil
