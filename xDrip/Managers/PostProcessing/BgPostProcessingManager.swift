...
class BgPostProcessingManager {
    ...
    @discardableResult
    func processBgReadings(processingStartDateOverride: Date?, fiveMinuteReadingsStartTimeStampOverride: Date? = nil, forceFullDownstreamRewrite: Bool = false, allowHistoricalDownstreamRewrite: Bool = false, onlyNewHistoricalReadingIDs: Set<String>? = nil) -> Bool {
        ...
        if processingStartDateOverride == nil || onlyNewHistoricalReadingIDs != nil {
            let contextOnlyBgReadings = bgReadings.filter { bgReading in
                // Watch history may use existing readings as calculation context, but it must not
                // rewrite the phone's authoritative history, current value or current trend.
                if let onlyNewHistoricalReadingIDs {
                    return !onlyNewHistoricalReadingIDs.contains(bgReading.id)
                }
        ...
        let downstreamReadingsToDelete = rebuildFiveMinuteCadenceFromStart &&
        ...
        if !downstreamReadingsToReplace.isEmpty || !downstreamReadingsToDelete.isEmpty {
            nightscoutSyncManager?.replaceBgReadingsInNightscout(
                bgReadings: downstreamReadingsToReplace,
                timeStampsToDelete: downstreamReadingsToDelete.map(\.timeStamp),
                blocksDirectLiveUpload: shouldRewriteFullDownstreamWindow)
            healthKitManager?.deleteBgReadingsFromHealthKit(bgReadingIDs: downstreamReadingsToDelete.map(\.id))
