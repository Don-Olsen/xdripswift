...
final class TroubleshootingLogTests: XCTestCase {
    ...
    func testActivityLogFilterMatchesRenderedMessagesAndRestoresFullListForBlankText() {
        ...
        // 7.1.1 renders units in the report header, not each accepted-reading row.
        XCTAssertEqual(report.message(for: entries[2]), "New reading: 123 at 07:59:58.")
        XCTAssertEqual(report.entries(matching: "reading: 123 at").map(\.kind), [
            entries[2].kind
        ])
